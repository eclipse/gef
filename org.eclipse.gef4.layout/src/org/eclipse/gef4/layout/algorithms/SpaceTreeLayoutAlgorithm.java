...
public class SpaceTreeLayoutAlgorithm implements LayoutAlgorithm {
	...
	private class SpaceTreeNode extends TreeLayoutObserver.TreeNode {
		...
		public SubgraphLayout collapseAllChildrenIntoSubgraph(
			...
			while (!nodesToVisit.isEmpty()) {
				...
				for (Iterator<TreeNode> iterator = currentNode.children
		...
		public void refreshSubgraphLocation() {
			if (subgraph != null && LayoutPropertiesHelper.isGraphEntity(subgraph)) {
				...
				LayoutPropertiesHelper.setLocation(subgraph, x, y);
			}
		...
		public double spaceRequiredForNode() {
			...
			case TOP_DOWN:
			case BOTTOM_UP:
				return LayoutPropertiesHelper.getSize(node).width;
			case LEFT_RIGHT:
			case RIGHT_LEFT:
				return LayoutPropertiesHelper.getSize(node).height;
			}
		...
		public boolean flushLocationChanges(double thicknessSoFar) {
			...
			if (expanded && subgraph == null) {
				thicknessSoFar += (depth >= 0 ? spaceTreeLayers.get(depth).thickness
						: 0)
						+ layerGap;
				for (Iterator<TreeNode> iterator = children.iterator(); iterator
						.hasNext();) {
	...
	private class SpaceTreeLayer {
		...
		public void removeNodes(List<TreeNode> nodesToRemove) {
			if (this.nodes.removeAll(nodesToRemove)) {
				...
				for (Iterator<TreeNode> iterator = nodesToRemove.iterator(); iterator
						.hasNext();) {
		...
		public void checkThickness(SpaceTreeNode node) {
			...
			Dimension size = LayoutPropertiesHelper.getSize(node.node);
			nodeThickness = (direction == TOP_DOWN || direction == BOTTOM_UP) ? size.height
					: size.width;
			if (node.subgraph != null
					&& LayoutPropertiesHelper.isGraphEntity(node.subgraph)) {
				size = LayoutPropertiesHelper.getSize(node.subgraph);
				nodeThickness += (direction == TOP_DOWN || direction == BOTTOM_UP) ? size.height
						: size.width;
			}
		...
		public void fitNodesWithinBounds(List<TreeNode> nodeList,
			...
			SpaceTreeNode[] nodes = nodeList.toArray(new SpaceTreeNode[nodeList
					.size()]);
		...
		private void collapseNode(SpaceTreeNode node) {
			...
			for (Iterator<TreeNode> iterator = node.children.iterator(); iterator
					.hasNext();) {
				SpaceTreeNode child = (SpaceTreeNode) iterator.next();
				if (child.expanded)
					collapseNode(child);
	...
	private class SpaceTreeExpandCollapseManager implements
		...
		public void setExpanded(LayoutContext context, NodeLayout node,
			...
			if (expanded) {
				...
				spaceTreeLayers.get(spaceTreeNode.depth + 1).removeNodes(
						spaceTreeNode.children);
				refreshLayout(animate);
			}
		}
		...
		public void maximizeExpansion(SpaceTreeNode nodeToExpand) {
			...
			spaceTreeLayers.get(nodeToExpand.depth + 1).removeNodes(
			...
			for (int layer = 0; !nodesInNextLayer.isEmpty(); layer++) {
				...
				for (Iterator<TreeNode> iterator = nodesInThisLayer.iterator(); iterator
						.hasNext();) {
					SpaceTreeNode node = (SpaceTreeNode) iterator.next();
				...
				for (Iterator<TreeNode> iterator = nodesInNextLayer.iterator(); iterator
						.hasNext();) {
					SpaceTreeNode node = (SpaceTreeNode) iterator.next();
					node.expanded = false;
				}
	...
	private int getSubgraphDirection() {
		switch (direction) {
		case TOP_DOWN:
			return ILayoutProperties.DIRECTION_TOP_DOWN;
		case BOTTOM_UP:
			return ILayoutProperties.DIRECTION_BOTTOM_UP;
		case LEFT_RIGHT:
			return ILayoutProperties.DIRECTION_LEFT_RIGHT;
		case RIGHT_LEFT:
			return ILayoutProperties.DIRECTION_RIGHT_LEFT;
		}
	...
	private double getAvailableSpace() {
		...
		for (Iterator<SpaceTreeLayer> iterator = spaceTreeLayers.iterator(); iterator
				.hasNext();) {
