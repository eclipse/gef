 ...
 * @author anyssen
 */
// TODO:refactor this to support merge between graphs, not builders
// TODO: the comment is still Zest/dot specific
final public class GraphMerger {

	private Graph sourceGraph;
	// TODO: this should not be given
	 ...
	 *            for nodes.
	 */
	public GraphMerger(Graph sourceGraph, String attributeNameForId) {
		this.sourceGraph = sourceGraph;
		this.attributeNameForId = attributeNameForId;
	}

	private Edge copy(Edge edge, Graph.Builder targetGraph, Map<Node, Node> copiedNodes, Map<Object, Node> nodesByIds) {
		// determine source and target
		Node srcSource = edge.getSource();
		Node source = find(nodesByIds, srcSource);
		if (source == null) {
			source = copiedNodes.get(srcSource);
		}

		Node srcTarget = edge.getTarget();
		Node target = find(nodesByIds, srcTarget);
		if (target == null) {
