...
/**
 * The {@link PanOnTypePolicy} is an {@link IOnTypePolicy} that performs
 * viewport panning via the keyboard.
 *
 * @author mwienand
 *
 */
public class PanOnTypePolicy extends AbstractInteractionPolicy
	...
	public void finalRelease(KeyEvent event) {
		...
		if (isRunning && !isDown && !isUp && !isLeft && !isRight) {
			...
			ChangeViewportPolicy viewportPolicy = getHost().getRoot()
					.getAdapter(ChangeViewportPolicy.class);
