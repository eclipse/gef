...
public class GeometricModel {
	...
	// selection handles
	private final GeometricShape topLeftSelectionHandle = new GeometricShape(createHandleShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 12, 15), Color.WHITE, GEF_SHADOW_EFFECT);
	private final GeometricShape topRightSelectionHandle = new GeometricShape(createHandleShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 243, 15), Color.WHITE, GEF_SHADOW_EFFECT);

	private final GeometricShape bottomLeftSelectionHandle = new GeometricShape(createHandleShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 12, 109), Color.WHITE, GEF_SHADOW_EFFECT);

	private final GeometricShape bottomRightSelectionHandle = new GeometricShape(createHandleShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 243, 109), Color.WHITE, GEF_SHADOW_EFFECT);

	// selection bounds
	// TODO: add transform to the curves
	private final GeometricCurve selectionBoundsTopLine = new GeometricCurve(
			new Point[] { new Point(), new Point(140, 24), new Point() }, GEF_COLOR_GREEN, GEF_STROKE_WIDTH,
			GEF_DASH_PATTERN, null);

	private final GeometricCurve selectionBoundsLeftLine = new GeometricCurve(
			new Point[] { new Point(), new Point(19, 70), new Point() }, GEF_COLOR_GREEN, GEF_STROKE_WIDTH,
			GEF_DASH_PATTERN, null);

	private final GeometricCurve selectionBoundsBottomLine = new GeometricCurve(
			new Point[] { new Point(), new Point(140, 118), new Point() }, GEF_COLOR_GREEN, GEF_STROKE_WIDTH,
			new Double[] { 15d, 10d }, null);

	private final GeometricCurve selectionBoundsRightLine = new GeometricCurve(
	...
	private final GeometricShape gBaseShape = new GeometricShape(createGBaseShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 27, 22), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	private final GeometricShape gTopShape = new GeometricShape(createGTopShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 27, 22), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	private final GeometricShape gMiddleShape = new GeometricShape(createGMiddleShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 27, 22), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// e shape
	private final GeometricShape eShape = new GeometricShape(createEShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 100, 22), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// f shape
	private final GeometricShape fShape = new GeometricShape(createFShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 175, 22), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// gDotShape
	private final GeometricShape gDotShape = new GeometricShape(createDotShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 87, 104), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// eDotShape
	private final GeometricShape eDotShape = new GeometricShape(createDotShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 170, 104), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// fDotShape
	private final GeometricShape fDotShape = new GeometricShape(createDotShapeGeometry(),
			new AffineTransform(1, 0, 0, 1, 225, 104), GEF_COLOR_BLUE, GEF_SHADOW_EFFECT);

	// fDotShape
	private final GeometricShape cursorShape = new GeometricShape(createCursorShapeGeometry(),
	...
	public List<AbstractGeometricElement<? extends IGeometry>> getShapeVisuals() {
		return visualShapes;
	}
