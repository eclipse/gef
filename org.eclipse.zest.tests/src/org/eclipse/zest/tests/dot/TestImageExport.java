...
public class TestImageExport extends TestDotTemplate {
  ...
  public static String dotBinDir() {
    if (dotDir == null) {
      ...
      InputStream stream = TestImageExport.class.getResourceAsStream("test.properties"); //$NON-NLS-1$
      if (stream == null) {
        System.err.println("Could not load the test.properties file in directory of " //$NON-NLS-1$
        ...
        try {
          ...
          if (dotDir == null || dotDir.trim().length() == 0) {
            System.err.println("Graphviz DOT directory not set in test.properties file"); //$NON-NLS-1$
          } else
            stream.close();
        } catch (IOException e) {
          e.printStackTrace();
  ...
  protected void testDotGeneration(final Graph graph) {
    /*
     * The DotExport class wraps the simple DotTemplate class, so when we test DotExport, we also
     * run the test in the test superclass:
     */
    if (dotDir != null) {
