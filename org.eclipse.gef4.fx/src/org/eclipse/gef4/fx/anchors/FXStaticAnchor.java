...
public class FXStaticAnchor extends AbstractFXAnchor {
     ...
     * coordinate system of the anchorage {@link Node}) for all attached {@link AnchorKey}s (i.e anchored {@link Node}
     * s), after having transformed
     * them into scene coordinates. In case the anchorage {@link Node} or any of
     * its ancestors are changed in a way that will affect the position, the {@link #positionProperty()} will be
     * updated.
     *
     * @param anchorage
    ...
    /**
     * Creates an {@link FXStaticAnchor} that is not bound to an anchorage {@link Node} and will always provide the
     * passed in position (in scene
     * coordinates) for all attached {@link AnchorKey}s (i.e. anchored {@link Node}s).
     *
    ...
    protected void recomputePositions(Node anchored) {
        ...
        Point positionInScene =
                anchorage == null ? referencePosition : JavaFX2Geometry.toPoint(anchorage.localToScene(
                        referencePosition.x, referencePosition.y));
        Point positionInAnchoredLocal =
                JavaFX2Geometry.toPoint(anchored.sceneToLocal(positionInScene.x, positionInScene.y));
