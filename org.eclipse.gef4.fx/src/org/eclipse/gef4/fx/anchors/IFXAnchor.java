...
/**
 * A visual anchor that can provide positions for
 * attached {@link AnchorKey}s (i.e. anchored {@link Node}s) in local coordinates of the {@link AnchorKey}'s anchored
 * {@link Node}. An {@link IFXAnchor} may be bound to an anchorage {@link Node}. If this is the case, it will
 ...
 * have an effect on the position (in local coordinates of the anchored {@link Node}s).
 *
 * @author anyssen
 */
public interface IFXAnchor {
    ...
    /**
     * Provides a read-only property with the anchorage {@link Node} this {@link IFXAnchor} is attached to. The property
     * value may be <code>null</code> in case this {@link IFXAnchor} is not bound to an
     * anchorage {@link Node}.
     *
    ...
    /**
     * Provides a position for the given {@link AnchorKey}. The provided {@link AnchorKey} has to be attached to this
     * {@link IFXAnchor} (see {@link #attach(AnchorKey)}).
     *
     * @param key
     *            The {@link AnchorKey} to retrieve a position for. It has to be
     *            attached ({@link #attach(AnchorKey)}) to this {@link IFXAnchor} before.
     * @return The position for the given {@link AnchorKey} within local coordinates of the {@link AnchorKey}'s anchored
     *         {@link Node}.
     */
    Point getPosition(AnchorKey key);

    /**
     * Determines if the given {@link AnchorKey} is currently attached to this {@link IFXAnchor}.
     *
     * @param key
     *            The {@link AnchorKey} to test.
     * @return <code>true</code> if the given key is attached, otherwise <code>false</code>.
     */
     ...
     *
     * @return A read-only (map) property storing positions for all attached {@link AnchorKey}s.
     */
