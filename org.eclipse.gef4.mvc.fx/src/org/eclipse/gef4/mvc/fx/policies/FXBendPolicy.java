...
/**
 * The {@link FXBendPolicy} can be used to manipulate the points constituting an {@link IFXConnection}, i.e. its start
 * point, way points, and end point. When
 ...
 * <li>Reconnecting points to the {@link IVisualPart} under mouse when applicable.</li>
 * </ul>
 *
 * @author mwienand
 */
public class FXBendPolicy extends AbstractPolicy<Node> implements ITransactional {
    ...
    public void createWayPoint(int segmentIndex, Point mouseInScene) {
        ...
        Point2D mouseInLocal = hostVisual.sceneToLocal(mouseInScene.x, mouseInScene.y);

        // create new way point
        segmentIndex++;
        op.getNewLinks().add(
                segmentIndex,
                ((IFXConnection) hostVisual).createWayPointAnchorLink(new Point(mouseInLocal.getX(), mouseInLocal
                        .getY())));
    ...
    protected AbstractFXContentPart getAnchorPart(List<IContentPart<Node>> partsUnderMouse) {
    ...
    protected void hideShowOverlaid() {
        ...
        if (linkIndex > 0) {
            ...
            if (getPosition(op.getNewLinks().get(prevIndex)).getDistance(currentPoint) < REMOVE_THRESHOLD) {
                // remove previous
        ...
        if (removedLinkIndex == -1 && linkIndex < op.getNewLinks().size() - 1) {
            ...
            if (getPosition(op.getNewLinks().get(nextIndex)).getDistance(currentPoint) < REMOVE_THRESHOLD) {
                // remove next
    ...
    public void movePoint(Point mouseInScene, List<IContentPart<Node>> partsUnderMouse) {
        // update position
        Point2D mouseInLocal = getHostVisual().sceneToLocal(mouseInScene.x, mouseInScene.y);
        Point2D startPointInLocal = getHostVisual().sceneToLocal(startPointInScene);
        Point delta =
                new Point(mouseInLocal.getX() - startPointInLocal.getX(), mouseInLocal.getY()
                        - startPointInLocal.getY());
    ...
    public void selectPoint(int segmentIndex, double segmentParameter, Point mouseInScene) {
    ...
    protected void updateCurrentAnchorLink(Point mouseInScene, List<IContentPart<Node>> partsUnderMouse) {
        // snaps for start and end (TODO: configurable)
        boolean snaps = linkIndex == 0 || linkIndex == op.getNewLinks().size() - 1;
        ...
        if (snaps) {
            ...
            if (anchorPart != null) {
                ...
                link = new AnchorLink(anchor, op.getOldLinks().get(linkIndex).getKey());
            }
        }
        ...
        if (link == null) {
            ...
            FXStaticAnchor anchor =
                    new FXStaticAnchor(JavaFX2Geometry.toPoint(getHostVisual().localToScene(currentPoint.x,
                            currentPoint.y)));
